...
新製品発表後に株価が上がった／下がったという一回の反応だけでは、その動きがAppleの発表イベントに典型的なパターンなのか、今回固有の材料なのかを判断できません。

そこで、まず過去5年間のiPhone発表前後を同じ視点で並べ、**発表当日とその後数週間〜四半期でどのようなパターンが繰り返されてきたか**を確認します。そのうえで今回のDuo発表を過去のパターンと比較し、もし違いがあれば価格設定、需要期待、マージン、供給など今回固有の材料を調べます。

最後に、発表直後の株価反応だけで結論を出さず、実売・出荷や利益率など次に確認すべきデータへつなげます。つまり、**過去のイベントパターン → 今回の反応 → 差を生んだ要因 → 次の検証項目**という順番で読みます。
...
まず過去のイベントを並べるのは、今回の値動きを評価するための基準を作るためです。発表当日の反応と、その後の数週間の動きを分けて見ることで、「イベント当日」と「実売・業績への評価」を混同しないようにします。
...
過去の基準を確認したら、次に今回の反応がその範囲内か、何が違うかを見ます。今回は発表当日はほぼ無風だった一方、翌日に上昇しているため、単純な「sell the news」だけでは説明しにくい点を掘り下げます。
...
- **強気の需要見通し:** Morgan Stanleyは12月四半期にDuoが約140億ドル（iPhone売上の16%）寄与と試算。Dan Ives氏は「4年間アップグレードしていない3億台の買い替え需要」を指摘し、DuoがいずれiPhone収益の20%を担う可能性に言及。

### 反証・留意点

ポジティブな株価反応を確認した後は、その解釈を弱める材料も確認します。価格を抑えて販売数量が伸びても、利益率や供給制約が悪化すれば業績への含意は変わるためです。
...
製品発表イベントでは、当日の株価だけで「成功／失敗」を判断せず、まず過去の同種イベントと比べ、今回だけの差分を探します。その差分が価格・需要・供給・利益率のどこから来ているかを確認し、その後の実績データで仮説を更新する使い方ができます。
