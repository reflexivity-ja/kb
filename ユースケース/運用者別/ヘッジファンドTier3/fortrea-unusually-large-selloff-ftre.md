---
id: RX-USECASE-0003
title: "Fortrea：異例の大幅下落 (FTRE)"
type: use-case
persona: "Hedge Fund Tier 3"
insight_type: "Scenario Insight"
signal: "Bearish"
language: ja
locale: ja-JP
published: 2026-08-04
drafted: 2026-09-07
revised: null
status: draft
source_url: "https://reflexivity.com/app/kg-insight?insightId=a5e0b764-a45f-4f90-a3be-ad2c9109f241"
canonical_path: "use-cases/hedge-fund-tier-3/fortrea-unusually-large-selloff-ftre.md"
translation_status: review-needed
resource: "Reflexivity Insights Proof Set — Sales Enablement.docx"
---

# Fortrea：異例の大幅下落 (FTRE) — 弱気シグナル

> **レビュー用ドラフト** — 以下は日付時点のプラットフォーム出力です。利用前に現在の市場データと照合するか、例示目的としてご利用ください。

**ペルソナ:** ヘッジファンド Tier 3（小規模・新興）  
**インサイトタイプ:** シナリオ・インサイト  
**シグナル:** 弱気  
**対象インサイト日:** 2026年8月4日  
**初稿:** 2026年9月7日

## Relevant（関連性）

これは、ソロの創業者PMが下落後の意味を自力で定量化しにくい、カバレッジの薄い小型株の典型例です。

## Compelling（注目ポイント）

Reflexivityは過去14回の類似局面を定量化しています。類似する異常な売り局面の後、Fortreaの将来リターン中央値は1カ月で-13.15%、6カ月で-29.33%で、品質スコアは5/8でした。

## Insight（インサイト）

PMは数分で投資仮説を確認できます。過去の分布は明確に弱気方向へ傾いており、6カ月のP20は-63.22%、P80は-3.50%です。この情報を基に、銘柄を逆張りするか、底固めを待つかを判断できます。

## Resource

- **Source document:** `Reflexivity Insights Proof Set — Sales Enablement.docx`
- **Live Reflexivity insight:** [Reflexivityで開く](https://reflexivity.com/app/kg-insight?insightId=a5e0b764-a45f-4f90-a3be-ad2c9109f241)

---

[← ヘッジファンド Tier 3](README.md) · [ユースケース一覧](../../README.md)

ご質問や詳細については **gtm@reflexivity.com** までお問い合わせください。
